Write-Host "Entering script AppCmdOnTargetMachines.ps1 Version 238"
...
function Get-Netsh-Command {
    param (
        ...
        if ([string]::IsNullOrEmpty($hostnamePort)) { #case 1: Existing binding not found.  Run the netsh ADD command to bind the certificate.
            return [string]::Format("http add sslcert {4}={0}:{1} appid='{{{3}}}' certhash={2} certstorename=MY", $hostOrIp, $port, $certhash, [System.Guid]::NewGuid().toString(), $keyName)
            ...
            return [string]::Format("http update sslcert {4}={0}:{1} appid='{3}' certhash={2} certstorename=MY", $hostOrIp, $port, $certhash, $applicationId, $keyName) #TODO: this won't work with older versions of netsh, add something here to check the netsh version.
        } 
...
function Test-WebsiteExist
{
    ...
    $website = Invoke-ToolInternal -Filename $appCmdPath -Arguments $appCmdArgs
...
function Test-BindingExist
{
    ...
    $sites = Invoke-ToolInternal -Filename $appCmdPath -Arguments $appCmdArgs
...
{  
    ...
    $appPool = Invoke-ToolInternal -Filename $appCmdPath -Arguments $appCmdArgs
...
function Enable-SNI
{
    ...
    Invoke-ToolInternal -Filename $appCmdPath -Arguments $appCmdArgs -RequireExitCodeZero
}
...
function Invoke-AdditionalCommand
{
    ...
    foreach($appCmdCommand in $appCmdCommands)
    {
        if(-not [string]::IsNullOrWhiteSpace($appCmdCommand.Trim(' ')))
        {
            ...
            Invoke-ToolInternal -Filename $appCmdPath -Arguments $appCmdCommand -RequireExitCodeZero
        }
    }
}
...
function Test-ApplicationExist {
    ...
    $application = Invoke-ToolInternal -Filename $appCmdPath -Arguments $appCmdArgs 
...
function Test-VirtualDirectoryExist 
{
    ...
    $virtualDirectory = Invoke-ToolInternal -Filename $appCmdPath -Arguments $appCmdArgs
